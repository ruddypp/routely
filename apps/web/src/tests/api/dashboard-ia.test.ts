...
import { DASHBOARD_MODULES, DASHBOARD_NAV_GROUPS } from "../../components/dashboard/types";

describe("dashboard information architecture", () => {
  it("keeps solo-operator operations modules easy to scan", () => {
    expect(DASHBOARD_NAV_GROUPS.map((group) => group.label)).toEqual(["Operate", "Release", "Observe", "Data", "Node"]);
  ...
  it("uses honest local-to-one-VPS and deferred-state copy", () => {
    const copy = DASHBOARD_MODULES.map((module) => `${module.label} ${module.summary}`).join(" ").toLowerCase();

    expect(copy).toContain("one-vps");
